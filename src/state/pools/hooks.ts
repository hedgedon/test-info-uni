...
import { Transaction, TransactionTest, TransactionType, WhitelistPool } from 'types'
...
import { useActiveNetworkVersion, useClients, useGfxClient } from 'state/application/hooks'
...
export function useUpdatePoolData(): (pools: PoolData[]) => void {
  ...
  const [network] = useActiveNetworkVersion()
  console.log('useUpdatePoolData(): called updatePoolData()')
  return useCallback(
...
export function usePoolDatas(poolAddresses: string[]): PoolData[] {
  ...
  // console.log('logging poolsWithData:', poolsWithData)
...
/*/// ////
 TESTING GFX ENDPOINTS BELOW
///// /// //////////
*/

export function usePoolTransactionsGfx(address: string): TransactionTest[] | undefined {
  const dispatch = useDispatch<AppDispatch>()
  const [activeNetwork] = useActiveNetworkVersion()
  const pool = useSelector((state: AppState) => state.pools.byAddress[activeNetwork.id]?.[address])
  const transactions = pool?.transactionsTest
  const [error, setError] = useState(false)
  const gfxClient = useGfxClient()

  useEffect(() => {
    async function fetch() {
      const { error, data } = await fetchPoolTransactionsGfx(address, gfxClient)
      if (error) {
        setError(true)
      } else if (data) {
        dispatch(
  ...
  }, [address, dispatch, error, transactions, gfxClient, activeNetwork.id])
