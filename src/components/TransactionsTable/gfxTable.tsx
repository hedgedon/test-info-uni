...
import { Transaction, TransactionType, TransactionTest } from 'types'
...
const DataRow = ({ transaction, color }: { transaction: TransactionTest; color?: string }) => {
...
export default function TransactionTableGfx({
  transactions,
  maxItems = 10,
  color,
}: {
  ...
  const sortedTransactions = useMemo(() => {
    return transactions
      ? transactions
          ...
          .sort((a, b) => {
            if (a && b) {
              return a[sortField as keyof TransactionTest] > b[sortField as keyof TransactionTest]
                ? (sortDirection ? -1 : 1) * 1
                : (sortDirection ? -1 : 1) * -1
            } else {
              return -1
